...
const Generator = argv => {
  ...
  let propsFileStr = undefined;

  try {
    propsFileStr = fs
      .readFileSync(`${templatePath}/__M_PROPS_FILE__.props.${type}`, "utf-8")
      .toString();
  } catch (error) {
    console.log("no exist *.props.ts");
  }
  ...
  if (propsFileStr) {
    const computedPropsStr = propsFileStr.replace(/__M_PROPS_FILE__/gi, folderName + "Props");
    fs.writeFileSync(`./${folderName}/${folderName}.props.${type}x`, computedPropsStr);
  }
